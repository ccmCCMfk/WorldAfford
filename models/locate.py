...
class Net(nn.Module):
    ...
    def forward(self, exo, ego, aff_label, epoch):
        ...
        # print(ego_attn.shape)
        ego_cls_attn = ego_attn[:, :, 0, 1:].reshape(b, 6, h, w) # todo 
        ...
        aff_logits = self.gap(aff_cam).reshape(b, num_exo, self.aff_classes)*w1+self.gmp(aff_cam).reshape(b, num_exo, self.aff_classes)*w2
        ...
        aff_logits_ego = self.gap(ego_pred).view(b, self.aff_classes)*w3+self.gmp(ego_pred).view(b, self.aff_classes)*w4
