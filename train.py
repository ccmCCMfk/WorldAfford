...
# from lightning.fabric import Fabric # todo
...
if __name__ == '__main__':
    ...
    #fabric = Fabric(accelerator="cuda",precision="bf16-mixed") # todo
    #fabric.launch() # todo
    ...
    #TrainLoader = fabric.setup_dataloaders(TrainLoader) #todo
    ...
    TestLoader = torch.utils.data.DataLoader(dataset=testset,
                                             ...
                                             pin_memory=True)
    #TestLoader = fabric.setup_dataloaders(TestLoader)  # todo

    model = model(aff_classes=args.num_classes)
    model = model.cuda()
    model.train()
    optimizer, scheduler = get_optimizer(model, args)
    #model,optimizer = fabric.setup(model,optimizer)  #todo
